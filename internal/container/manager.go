...
func (m *Manager) GetRunning(agentID string) *ContainerInfo {
	...
	inspect, err := m.docker.ContainerInspect(ctx, info.ID, client.ContainerInspectOptions{})
	if err == nil && inspect.Container.State != nil && inspect.Container.State.Running {
		return info
	}

	m.mu.Lock()
	if current, ok := m.active[agentID]; ok && current.ID == info.ID {
		delete(m.active, agentID)
	}
	m.mu.Unlock()

	if err != nil {
		slog.Warn("tracked agent container unavailable", "agent", agentID, "container", info.ID[:12], "error", err)
	} else {
		status := ""
		if inspect.Container.State != nil {
			status = string(inspect.Container.State.Status)
		}
		slog.Warn("tracked agent container not running", "agent", agentID, "container", info.ID[:12], "status", status)
	}
	return nil
}
